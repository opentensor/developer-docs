...
Bittensor subnets operate as *constant product AMMs*, meaning that the price is adjusted so that any transaction must preserve the product of the two tokens in reserve, TAO and alpha. To calulate the price of batch of tokens that a buyer wishes to buy&mdash;alpha if they are staking, or TAO if they are unstaking&mdash;the algorithm assumes that the transaction does not change this product, so the product of TAO and alpha is the same before and after.
...
$$
\text{Yield} = 100 - \frac{ 10 * 100} {10 + 5}
...
This slippage is 50% of the actual swap value, which is extremely high, because we chose small values for the available liquidity. **In general, slippage is high when available liquidity is limited.**
147.18, 145880
